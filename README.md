...
```bash
# Google Gemini AI (server-only, required for AI analysis)
GEMINI_API_KEY=...
...
For local development, prefer copying `.env.local.example` to `.env.local` and filling in the values. Do not prefix `GEMINI_API_KEY` with `VITE_`; it must stay server-only.
